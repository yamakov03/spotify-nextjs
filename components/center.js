...
function Center() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (pl.tracks.total > pl.tracks.limit) {
        // Divide the total number of track by the limit to get the number of API calls
        for (let i = 1; i < Math.ceil(pl.tracks.total / pl.tracks.limit); i++) {
          const trackToAdd = (
  ...
  return (
    <div className="flex-grow h-[calc(100vh-5rem)] overflow-y-scroll">
      {/* header */}
      <section
        className={`flex items-end space-x-7 bg-gradient-to-b to-neutral-900 h-[400px] text-white pl-8 pr-8 pb-8 pt-6`}
        style={Object.assign(
