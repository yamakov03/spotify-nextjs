...
function SearchBarLg() {
  ...
  return (
    <div ref={inputRef} className={`group relative flex items-center rounded-full bg-[--searchbar-tinted-base] hover:bg-[--searchbar-tinted-press] h-[50px] w-[330px] text-sm border-[1px] hover:border-solid border-transparent 
      ${inputFocused ? 'border-white' : 'hover:border-[--background-tinted-highlight]'} `}>
        <svg className={`absolute left-[20px] group-hover:text-white ${searchVal ? 'text-white' : 'text-neutral-200'}`}
        ...
        <input
          ...
          className="w-full h-full py-4 ps-12 pe-12 bg-transparent outline-none text-[--home-text-light] placeholder-neutral-200"
        />
       <svg className={`absolute left-[300px] text-white ${searchVal ? 'visible' : 'invisible'}`}
