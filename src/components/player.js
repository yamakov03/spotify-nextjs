...
function Player() {
  ...
  const playNext = () => {
    if (playing.playlistId === "liked") {
      ...
      shuffle ?
        setPlaying({ ...playing, typePlaying: "track", trackOrder: index, trackId: likedSongs?.items[index]?.track?.id, isPlaying: true })
        :
        setPlaying({ ...playing, typePlaying: "track", trackOrder: playing.trackOrder + 1, trackId: playing.trackOrder === likedSongs?.items.length ? null : likedSongs?.items[playing.trackOrder + 1]?.track?.id, isPlaying: true })
    } else if (playing.typePlaying === "track") {
      var index = Math.floor(Math.random() * (playlist?.tracks.items.length - 1 - 0) + 0);
      shuffle ?
        setPlaying({ ...playing, typePlaying: "track", trackOrder: index, trackId: playlist?.tracks?.items[index]?.track?.id, isPlaying: true })
        :
        setPlaying({ ...playing, typePlaying: "track", trackOrder: playing.trackOrder + 1, trackId: playing.trackOrder === playlist?.tracks.items.length ? null : playlist?.tracks?.items[playing.trackOrder + 1]?.track?.id, isPlaying: true })
    }
  }
  ...
  return (
    <div onLoad={() => {
      ...
      var observer = new MutationObserver(function (mutations) {
        if (target.getAttribute('data-position') === '99.9' || target.getAttribute('data-position') === '100') {
          playNext();
        }
      });
        ...
        components={{
          ...
          leftButton:
            <div className="flex align-middle justify-center">
              <button onClick={() => { handleShuffle() }} className={`hover:text-[#d9d9d9] relative left-[-10px] `}>
